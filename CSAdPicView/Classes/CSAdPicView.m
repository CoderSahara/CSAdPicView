...
@interface CSAdPicView ()<UIScrollViewDelegate>
{
    /**< 当前在哪一个位置*/
...
@implementation CSAdPicView

+ (instancetype)picViewWithLoadImageBlock: (LoadImageBlock)loadBlock {
    
    CSAdPicView *adView = [[CSAdPicView alloc] init];
...
-(void)setPicModels:(NSArray<id <CSAdPicProtocol>> *)picModels
{
    ...
    for (int i = 0; i< count; i++) {
        ...
        id<CSAdPicProtocol> picM = picModels[i % baseCount];
        // 1. 创建控件
...
- (void)jumpToLink:(UITapGestureRecognizer *)gester {
    ...
    id<CSAdPicProtocol> adM = self.picModels[tag];
